                        ...
                        <div v-else-if="punks.length > 0" class="row">
                            <div v-for="(punk) in punks" v-bind:key="punk.idx" style="margin-bottom: 10px;" class="col" @click="showDetail(punk.idx)">
                                <div style="height: 100%; max-width: 300px; min-width: 210px; margin: 0 auto;" class="card">
                                    ...
                                    <img v-if="crypto_Punks.indexOf(punk.idx) > -1" class="card-img-top pixelated" :src="('/crypto/' +punk.idx +'-CrossPunks.png')">
                                    <img v-else class="card-img-top pixelated" :src="(`https://crosspunks.com/images/crosspunks/${punk.idx}.png`)">
                                    <div  class="card-body">
                                        ...
                                        <div>
                                            <div class="text-muted card-subtitle h6">
                                                ...
                                                <span v-if="punk.bid && punk.bid.isForSale == true" style="float: right">
                                                    {{ walletManager.tronWebGlobal.BigNumber(punk.bid.minValue.hex ? punk.bid.minValue.hex : punk.bid.minValue).toNumber() / 1000000 }}
                                                ...
                                                <span v-if="punk.real_bid && punk.real_bid.hasBid" style="float: right; margin-top: 5px;">
                                                    {{(walletManager.tronWebGlobal.BigNumber(punk.real_bid.value.hex ? punk.real_bid.value.hex : punk.real_bid.value).toNumber()) / 1000000 }}
            ...
            async getMyPunks(){
                ...
                if(!this.punk_loading) {
                    ...
                    try {
                        ...
                        try {
                            let dataServer = await this.$http.get(`https://crosspunks.com/server/myPunks?wallet=` + this.walletManager.ttronWeb.defaultAddress.base58)
                            let rows = JSON.parse(dataServer.data.msg);
                            if(rows.length > 0) {
                                loadFromServer = true;
                                for(let rowId in rows){
                    ...
                    this.allPunks.sort((a, b) => {
                        if (sortBy === "token_id_lowest") {
                            ...
                            if (!this.price_idx[a.idx])
                                this.price_idx[a.idx] = this.walletManager.tronWebGlobal.BigNumber(a.bid.minValue.hex ? a.bid.minValue.hex : a.bid.minValue).toNumber();

                            let first = this.price_idx[a.idx];

                            if (!this.price_idx[b.idx])
                                this.price_idx[b.idx] = this.walletManager.tronWebGlobal.BigNumber(b.bid.minValue.hex ? b.bid.minValue.hex : b.bid.minValue).toNumber();
                            ...
                            if (!this.price_bid_idx[a.idx])
                                this.price_bid_idx[a.idx] = this.walletManager.tronWebGlobal.BigNumber(a.real_bid.value.hex ? a.real_bid.value.hex : a.real_bid.value).toNumber();

                            let first = this.price_bid_idx[a.idx];

                            if (!this.price_bid_idx[b.idx])
                                this.price_bid_idx[b.idx] = this.walletManager.tronWebGlobal.BigNumber(b.real_bid.value.hex ? b.real_bid.value.hex : b.real_bid.value).toNumber();
