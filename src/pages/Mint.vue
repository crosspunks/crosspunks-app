...
export default {
  ...
  methods: {
      async getPunk(){
          if(!this.btn_loading) {
              ...
              try {
                  ...
                  // await this.walletManager.contract.mintNFTAirDrop(this.crosspunks_count, (invite_code > 0 ? invite_code : 0)).send({
                  await this.walletManager.contract.mintNFT(this.crosspunks_count).send({
                      feeLimit: fee,
                      callValue: (this.crosspunks_count * 1000000000),
      ...
      async loadPunkLeft(){
          setTimeout(async ()=>{
              let number = await this.walletManager.contractGlobal.totalSupply().call();
              this.punkLeft = 10000 - (this.walletManager.tronWebGlobal.BigNumber(number).toNumber())
          }, 1000);
